...
PATH = r"C:\Users\prakh\OneDrive\Documents\PROJECT\aclImdb"
...
def dataprocessing(x):
    ...
    processed_text = ' '.join([re.sub(r'\d+', '', i) for i in word_tokenize(x)])
    return x

reviews_train['review'] = reviews_train['review'].apply(dataprocessing)
reviews_test['review'] = reviews_test['review'].apply(dataprocessing)
